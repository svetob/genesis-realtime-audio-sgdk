...
#define XGM2PCM_FAST_BUS_ACCESS

// ===========================
// PRIVATE
// ===========================

extern void XGM2PCM_mixIntoRingBuffer(s8 *pcm, vu8 *ringbuf);
extern void XGM2PCM_mixIntoRingBuffer_withOverflowProtection_ASM(s8 *pcmBuf,
                                                                 XGM2PCMMixerStatus *status);
extern void XGM2PCM_overwriteRingBuffer_ASM(s8 *pcmBuf, XGM2PCMMixerStatus *status);

#ifdef XGM2PCM_FAST_BUS_ACCESS
...
static inline void XGM2PCM_Z80_getAndRequestBus_fast()
{
...
static inline void XGM2PCM_Z80_releaseBus_fast()
{
...
{
    intsPrev = XGM2PCM_SYS_disableInts_fast_noStack();
    XGM2PCM_Z80_getAndRequestBus_fast();
}

static inline void exitBus()
{
    XGM2PCM_Z80_releaseBus_fast();
    XGM2PCM_SYS_enableInts_fast_noStack(intsPrev);
}
...
u8 XGM2PCM_peek_ringbuf_writepos()
{
    ...
    u8 pos = *XGM2PCM_RINGBUF_WRITEPOS_VAR;
...
u8 XGM2PCM_peek_pcm_channel_status()
{
    ...
    u8 pcmPlaying = status & XGM2PCM_PLAYING_MASK;
...
void XGM2PCM_mix_into_ringbuf(void *pcmSource512, XGM2PCMMixerStatus *mixerStatus)
{
    ...
    *XGM2_DAC_ENABLED_CNT = 0x04;
    ...
    u8 pcmPlaying = xgm2Status & XGM2PCM_PLAYING_MASK;
    ...
    if (ringWritePos != mixerStatus->ringPosPrev) {
        ...
        if (shouldProtect) {
            ...
            XGM2PCM_overwriteRingBuffer_ASM(pcmSource512, mixerStatus);
        }
    }
